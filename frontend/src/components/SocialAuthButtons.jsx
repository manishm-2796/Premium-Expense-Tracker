...
export default function SocialAuthButtons() {
  ...
  const handleGoogleSignIn = () => {
    // If Google Client ID is configured, use real OAuth
    ...
    // Fallback: use backend social-login endpoint directly
    handleSocialFallback('google', 'Google');
  };

  // ── Generic social login fallback (uses backend /auth/social-login) ──
  const handleSocialFallback = async (provider, displayName) => {
    setLoadingProvider(provider);
    setError('');
    try {
      const email = prompt(`Enter your ${displayName} email to sign in:`);
      ...
      await socialLogin({
        ...
        token: `${provider}_${Date.now()}`
      });
      navigate('/dashboard');
    } catch (err) {
      setError(`${displayName} sign-in failed. Please try again.`);
    } finally {
      setLoadingProvider(null);
    }
  };
  ...
  const socialPlatforms = [
    ...
    },
    {
      ...
      onClick: () => handleSocialFallback('facebook', 'Facebook'),
      icon: (
    ...
    },
    {
      ...
      onClick: () => handleSocialFallback('instagram', 'Instagram'),
      icon: (
    ...
    },
    {
      ...
      onClick: () => handleSocialFallback('x', 'X (Twitter)'),
      icon: (

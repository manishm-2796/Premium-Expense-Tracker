...
export default function SocialAuthButtons() {
  ...
  const handleSocialSignIn = async (provider) => {
    ...
    try {
      // Mock social authentication data / OAuth token exchange
  ...
  return (
    <div style={{ marginTop: '1.5rem' }}>
      ...
      {error && <div className="error-msg" style={{ marginBottom: '1rem', fontSize: '0.85rem' }}>{error}</div>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
        ...
        >
          ...
          {loadingProvider === 'instagram' ? 'Connecting...' : 'Instagram'}
        </button>
        ...
        >
          ...
          {loadingProvider === 'x' ? 'Connecting...' : 'X (Twitter)'}
        </button>
      </div>
    </div>
  );
